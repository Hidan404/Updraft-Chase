...
function barreiras(altura, largura, abertura, espaco, notificarPonto){
    ...
    this.animarLoop = () => {
        this.pares.forEach(par => {
            ...
            const atravessandoMeio = par.getX() + movimentoGradual >= meioDaTela && par.getX() < meioDaTela
...
const colisaoTest = (personagem, barreiras) => {
    return barreiras.pares.some(parBarreiras => {
        ...
        return sobreposicaoSouN(personagem.elemento, superior) || sobreposicaoSouN(personagem.elemento, inferior);
    });
}
...
const flappyBird = () => {
    ...
    const barreirasS = new barreiras(altura, largura, 200, 400, () => progresso.atualizarPontos(++pontos))
        ...
        temporizadorJogo = setInterval(() => {
            if (!jogoParado) {
                ...
                if (colisaoTest(persona, barreirasS)) {
                    ...
                    jogoParado = true;  // Sinaliza que o jogo parou
                    clearInterval(temporizadorJogo);  // Para o loop do jogo
